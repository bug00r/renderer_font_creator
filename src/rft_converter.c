...
void __rft_print_header(rft_pcache_t *cache) {
	...
	fprintf(cache->out_file,"#define _N NULL\n");
	fprintf(cache->out_file,"#define _SV static Vec2\n");
	fprintf(cache->out_file,"#define _SG static rf_glyph_t\n\n");
}
...
void __rft_print_glyph_not_found(rft_pcache_t *cache)
{
	...
	fprintf(out_file, "_SV oPts_empty[] = {{0.f,0.f}, {0.f,100.f},{100.f,100.f},{100.f,0.f},{0.f,0.f},{10.f,10.f},{90.f,10.f},{90.f,90.f},{10.f,90.f},{10.f,10.f}};\n");
	fprintf(out_file, "_SG __rf_glyph_empty = {{0,0,100,100},10,&oPts_empty[0]};\n\n");
}

void __rft_print_glyph_container(rft_pcache_t *cache)
{
	FILE *out_file = cache->out_file;
	fprintf(out_file, "\nstatic rf_glyph_t* __rf_gl_container_get_fn(unsigned long charcode)\n{\n\trf_glyph_t* found = ( charcode <= %lu ? __rfgs[charcode] : &__rf_glyph_empty);\n\n\tif( found == NULL )\n\t{\n\t\tfound = &__rf_glyph_empty;\n\t}\n\n\treturn found;\n}\n\n", cache->endCode);
...
void __rft_print_glyph_provider(rft_pcache_t *cache)
{
	...
	fprintf(cache->out_file,"#undef _N\n");
	fprintf(cache->out_file,"#undef _SV\n");
	fprintf(cache->out_file,"#undef _SG\n");
}
...
static void __rft_add_bezier_point(rft_pcache_t * _cache, Vec2 const * const p2)
{
	rft_pcache_t *cache = _cache;
...
int __rft_conic_to( const FT_Vector*  control, const FT_Vector*  to, void* data ) 
{
	...
	Vec2 start = { (float)lastPoint->x, (float)lastPoint->y };
	Vec2 cp = { (float)control->x, (float)control->y };
	Vec2 end = { (float)to->x, (float)to->y };
	__rft_set_last_pt(cache, to);
	uint32_t steps = 5;
...
int __rft_cubic_to( const FT_Vector*  control1, const FT_Vector*  control2, const FT_Vector*  to, void* data ) 
{
	...
	Vec2 start = {  (float)lastPoint->x, (float)lastPoint->y };
	Vec2 cp1 = {  (float)control1->x, (float)control1->y };
	Vec2 cp2 = { (float)control2->x, (float)control2->y };
	Vec2 end = {  (float)to->x, (float)to->y };
	__rft_set_last_pt(cache, to);
	uint32_t steps = 5;
...
static void __rft_print_glyph_entry(FILE *_out_file, rft_outlines_t *_outlines, size_t cntPoints)
{
	...
	fprintf(out_file, "_SG _rfg%lx = {{%li,%li,%li,%li},%lli,&oPts%lx[0]};", 
			outlines->charcode, bbox->xMin, bbox->yMin, bbox->xMax, bbox->yMax, cntPoints, outlines->charcode);
}
...
static void __rft_print_outlines(void **data, void *eachdata)
{
	...
	if ( outlines->outline_list != NULL )
	{
		...
		fprintf(out_file, "_SV oPts%lx[] = {", outlines->charcode);

		dl_list_each_data(outlines->outline_list, &rft_op, __rft_print_outline);

		if (rft_op.cntPt == 0)
		{
			fprintf(out_file, "{0.f,0.f}");
		}
		...
		__rft_print_glyph_entry(out_file, outlines, rft_op.cntPt);
	}
}
...
static void __rft_print_glyph_entries(void **data, void *eachdata)
{
	...
	if ( outlines->outline_list != NULL )
	{
		fprintf(out_file, "&_rfg%lx,", outlines->charcode);
	} else {
		fprintf(out_file, "_N,");
	}
}
...
static void __rft_print_glyphs_array(rft_pcache_t *_pCache, rft_glyphs_t * glyphs)
{
	...
	fprintf(pCache->out_file, "_SG* __rfgs[%lu] = {", pCache->endCode + 1);
